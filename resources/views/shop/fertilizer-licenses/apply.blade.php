...
@section('content')
    <div class="max-w-5xl mx-auto py-6 sm:py-8 px-0 sm:px-4">
        <div class="mb-8">
            ...
            <p class="text-sm text-slate-600 mt-2 max-w-3xl">Submit your license application with NRC attachments and
                fertilizer items in one place. You can add or remove items before submitting.</p>
        </div>
                ...
                <div class="flex flex-wrap items-center justify-between gap-3">
                    <div>
                        ...
                        <p class="text-emerald-700 mt-1">
                            {{ $latestLicense->application_date?->format('M j, Y') ?? 'No application date recorded' }} ·
        ...
        <form action="{{ route('shop.fertilizer-licenses.store') }}" method="POST" enctype="multipart/form-data"
            class="space-y-6">
            @csrf
            ...
            <div class="bg-white rounded-3xl border border-emerald-100 shadow-sm p-6 sm:p-8 space-y-6">
                <div class="grid sm:grid-cols-2 gap-5">
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-1" for="application_date">Application
                            Date</label>
                    ...
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-1" for="applicant_name">Applicant
                            Name</label>
                    ...
                    <div>
                        ...
                        <input type="text" name="nrc_number" id="nrc_number" value="{{ old('nrc_number') }}"
                ...
                <div class="grid sm:grid-cols-2 gap-5">
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-1" for="education_level">Education
                            Level</label>
                    ...
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-1" for="work_experience">Work
                            Experience</label>
                ...
                <div class="grid sm:grid-cols-2 gap-5">
                    <div>
                        ...
                        <select name="township" id="township"
                            class="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm focus:ring-2 focus:ring-emerald-500 outline-none @error('township') border-red-400 @enderror">
                            <option value="Hinthada">Hinthada</option>
                        </select>
                        @error('township')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-1" for="permanent_address">Permanent
                            Address</label>
                        <textarea name="permanent_address" id="permanent_address" rows="3"
                            class="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm focus:ring-2 focus:ring-emerald-500 outline-none @error('permanent_address') border-red-400 @enderror">{{ old('permanent_address') }}</textarea>
                        @error('permanent_address')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-1"
                            for="distribution_location_address">Distribution Location Address</label>
                        <textarea name="distribution_location_address" id="distribution_location_address" rows="3"
                            class="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm focus:ring-2 focus:ring-emerald-500 outline-none @error('distribution_location_address') border-red-400 @enderror">{{ old('distribution_location_address') }}</textarea>
                        @error('distribution_location_address')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                </div>
                ...
                <div class="grid sm:grid-cols-2 gap-5">
                    <div>
                        ...
                        <input type="text" name="building_type" id="building_type" value="{{ old('building_type') }}"
                            class="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm focus:ring-2 focus:ring-emerald-500 outline-none @error('building_type') border-red-400 @enderror">
                        @error('building_type')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-1" for="building_dimensions">Building
                            Dimensions</label>
                        <input type="text" name="building_dimensions" id="building_dimensions"
                            value="{{ old('building_dimensions') }}"
                            class="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm focus:ring-2 focus:ring-emerald-500 outline-none @error('building_dimensions') border-red-400 @enderror">
            ...
            <div class="bg-white rounded-3xl border border-emerald-100 shadow-sm p-6 sm:p-8 space-y-5">
                <div class="flex items-center justify-between gap-3">
                    ...
                    <button type="button" id="addFertilizerRow"
                        class="px-4 py-2.5 rounded-xl bg-emerald-100 text-emerald-900 font-bold text-sm hover:bg-emerald-200 transition">Add
                        Row</button>
                </div>
                    ...
                    @foreach(['front' => 'Front NRC Image', 'back' => 'Back NRC Image'] as $side => $label)
                        <div>
                            ...
                            <input type="file" name="attachment_nrc[{{ $side }}]" id="attachment_nrc_{{ $side }}"
                                accept="image/*"
                                class="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm bg-white focus:ring-2 focus:ring-emerald-500 outline-none @error('attachment_nrc.' . $side) border-red-400 @enderror">
                            @error('attachment_nrc.' . $side)<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                        </div>
                    @endforeach
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-1"
                            for="township_recommendation_letter">Township Recommendation Letter</label>
                        <input type="file" name="township_recommendation_letter" id="township_recommendation_letter"
                            accept="image/*"
                            class="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm bg-white focus:ring-2 focus:ring-emerald-500 outline-none @error('township_recommendation_letter') border-red-400 @enderror">
                        @error('township_recommendation_letter')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                    </div>
                </div>
            </div>
            ...
            <div class="flex items-center justify-end gap-3 pb-6">
                <a href="{{ route('shop.dashboard') }}"
                    class="px-5 py-3 rounded-xl border border-slate-200 text-slate-600 font-bold hover:bg-slate-50">Cancel</a>
                <button type="submit"
                    class="px-6 py-3 rounded-xl bg-emerald-700 text-white font-black hover:bg-emerald-800 shadow-lg shadow-emerald-700/10">Submit
                    Application</button>
            </div>
        </form>
    </div>
