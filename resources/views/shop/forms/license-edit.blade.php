...
@section('title', 'ပြင်ဆင်ရန် - ပိုးသတ်ဆေးလုပ်ငန်းလိုင်စင်လျှောက်လွှာ')
@section('breadcumb')
    <a href="{{ route('shop.dashboard') }}" class="hover:underline">{{ __('messages.shop.dashboard') }}</a>
    <span>&middot;</span>
    <span>လိုင်စင်လျှောက်လွှာ ပြင်ဆင်ရန်</span>
@endsection

@section('content')
    <div class="w-[100%] max-w-4xl mx-auto py-8 px-4">
        <h1 class="text-2xl font-black text-slate-900 mb-1">ပိုးသတ်ဆေးလုပ်ငန်းလိုင်စင်လျှောက်လွှာ ပြင်ဆင်ရန် (ပုံစံ - ၇)</h1>
        <p class="text-sm text-slate-500 mb-6">Modify the necessary fields below to update your application. Fields without new file uploads will preserve their existing data safely.</p>
        ...
        <form id="license-form" action="{{ route('shop.licenseUpdate', $pesticideShop->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
            ...
            <div class="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm space-y-4">
                ...
                <div class="grid sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $pesticideShop->name) }}" required class="w-full rounded-xl border border-slate-200 px-4 py-2.5 focus:ring-2 focus:ring-emerald-500 outline-none">
                    </div>
                    <div>
                        <label for="township" class="block text-sm font-bold text-slate-700 mb-1">Township (မြို့နယ်)</label>
                        <input type="text" name="township" id="township" value="{{ old('township', $pesticideShop->township) }}" required class="w-full rounded-xl border border-slate-200 px-4 py-2.5 focus:ring-2 focus:ring-emerald-500 outline-none">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="flex flex-wrap gap-2 items-center">
                        ...
                        <select id="district" class="rounded-xl border border-slate-200 px-3 py-2.5 bg-white focus:ring-2 focus:ring-emerald-500 outline-none min-w-[120px]" data-old-value="{{ old('district_old_val', $nrcDistrict) }}">
                ...
                <div class="grid sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="education" id="education" value="{{ old('education', $pesticideShop->education) }}" required class="w-full rounded-xl border border-slate-200 px-4 py-2.5 focus:ring-2 focus:ring-emerald-500 outline-none">
                    </div>
                    <div>
                        <label for="stable_address" class="block text-sm font-bold text-slate-700 mb-1">Permanent Address (အမြဲတမ်းနေရပ်လိပ်စာ)</label>
                        <input type="text" name="stable_address" id="stable_address" value="{{ old('stable_address', $pesticideShop->stable_address) }}" required class="w-full rounded-xl border border-slate-200 px-4 py-2.5 focus:ring-2 focus:ring-emerald-500 outline-none">
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm space-y-4">
                ...
                <div>
                    ...
                    <textarea name="requested_selling_address" id="requested_selling_address" rows="2" required class="w-full rounded-xl border border-slate-200 px-4 py-2.5 focus:ring-2 focus:ring-emerald-500 outline-none">{{ old('requested_selling_address', $pesticideShop->requested_selling_address) }}</textarea>
                </div>
                ...
                <div class="grid sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="building_type" id="building_type" value="{{ old('building_type', $pesticideShop->building_type) }}" required class="w-full rounded-xl border border-slate-200 px-4 py-2.5 focus:ring-2 focus:ring-emerald-500 outline-none">
                    </div>
                    <div>
                        <label for="building_area" class="block text-sm font-bold text-slate-700 mb-1">Building Dimensions (အကျယ်အဝန်း)</label>
                        <input type="text" name="building_area" id="building_area" value="{{ old('building_area', $pesticideShop->building_area) }}" required class="w-full rounded-xl border border-slate-200 px-4 py-2.5 focus:ring-2 focus:ring-emerald-500 outline-none">
                    </div>
                </div>
                ...
                <div class="grid sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="from_restaurant_distance" id="from_restaurant_distance" value="{{ old('from_restaurant_distance', $pesticideShop->from_restaurant_distance) }}" required class="w-full rounded-xl border border-slate-200 px-4 py-2.5 focus:ring-2 focus:ring-emerald-500 outline-none">
                    </div>
                    <div>
                        ...
                        <select name="retail_or_wholesale" id="retail_or_wholesale" required class="w-full rounded-xl border border-slate-200 px-4 py-2.5 bg-white focus:ring-2 focus:ring-emerald-500 outline-none">
                            <option value="retail" {{ old('retail_or_wholesale', $pesticideShop->retail_or_wholesale) === 'retail' ? 'selected' : '' }}>Retail (လက်လီ)</option>
                            <option value="wholesale" {{ old('retail_or_wholesale', $pesticideShop->retail_or_wholesale) === 'wholesale' ? 'selected' : '' }}>Wholesale (လက်ကား)</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="flex items-start bg-slate-50 p-4 rounded-2xl border border-slate-100">
                    <div class="flex items-center h-5">
                        <input type="checkbox" name="has_emergency_preparedness_plan" id="has_emergency_preparedness_plan" value="1" {{ old('has_emergency_preparedness_plan', $pesticideShop->has_emergency_preparedness_plan) ? 'checked' : '' }} class="h-4 w-4 rounded border-slate-300 text-emerald-600 focus:ring-emerald-500">
                    </div>
            ...
            <div class="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm space-y-6">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 bg-slate-50/50 p-4 rounded-2xl border border-slate-100">
                    <div>
                        ...
                        <input type="text" name="surrounding_agreements[location][village]" value="{{ old('surrounding_agreements.location.village', data_get($surroundingAgreements, 'location.village')) }}" required class="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none bg-white focus:ring-2 focus:ring-emerald-500">
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-slate-500 mb-1">Village Tract (ကျေးရွာအုပ်စု)</label>
                        <input type="text" name="surrounding_agreements[location][village_tract]" value="{{ old('surrounding_agreements.location.village_tract', data_get($surroundingAgreements, 'location.village_tract')) }}" required class="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none bg-white focus:ring-2 focus:ring-emerald-500">
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-slate-500 mb-1">Township (မြို့နယ်)</label>
                        <input type="text" name="surrounding_agreements[location][township]" value="{{ old('surrounding_agreements.location.township', data_get($surroundingAgreements, 'location.township')) }}" required class="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none bg-white focus:ring-2 focus:ring-emerald-500">
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-slate-500 mb-1">Region/State (တိုင်း/ပြည်နယ်)</label>
                        <input type="text" name="surrounding_agreements[location][region_state]" value="{{ old('surrounding_agreements.location.region_state', data_get($surroundingAgreements, 'location.region_state')) }}" required class="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none bg-white focus:ring-2 focus:ring-emerald-500">
                    </div>
                </div>
                                ...
                                <div>
                                    ...
                                    @if($existingSig = data_get($surroundingAgreements, "boundaries.{$key}.signature"))
                                        <div id="preview-sig-{{ $key }}" class="mt-2 border border-slate-200 rounded-xl overflow-hidden bg-white max-h-24 flex items-center justify-start p-1 gap-2">
                                            <img src="{{ asset('storage/' . $existingSig) }}" alt="Existing Sig" class="max-h-20 w-auto object-contain rounded-lg">
                                            <span class="text-[10px] text-emerald-700 font-semibold">✓ လက်ရှိဖိုင်သိမ်းဆည်းပြီး</span>
                                        </div>
                                    @else
                                        <div id="preview-sig-{{ $key }}" class="hidden mt-2 border border-slate-200 rounded-xl overflow-hidden bg-white max-h-24 flex items-center justify-start p-1">
                                            <img src="" alt="Preview" class="max-h-20 w-auto object-contain">
                                        </div>
                                    @endif
                                </div>
                            </div>
                    ...
                    @foreach([
                        ...
                        <div class="p-4 bg-slate-50 rounded-2xl border border-slate-100 flex flex-col justify-between space-y-3">
                            <div>
                                ...
                                <p class="text-[10px] text-slate-400 mt-1">Leave blank to retain original attachment upload.</p>
                            </div>
                            
                            @if($existingAttachment = data_get($attachments, $fileKey))
                                <div id="preview-{{ $fileKey }}" class="border border-slate-200 rounded-xl overflow-hidden bg-white max-h-32 flex items-center justify-between p-2">
                                    <img src="{{ asset('storage/' . $existingAttachment) }}" alt="Preview" class="max-h-28 w-auto object-contain rounded-lg">
                                    <span class="text-[10px] text-emerald-700 font-bold px-2 py-1 bg-emerald-50 rounded-lg">✓ Current File</span>
                                </div>
                            @else
                ...
                <div class="max-w-xs">
                    ...
                    @if($pesticideShop->signature)
                        <div id="preview-main-sig" class="mt-3 border border-slate-200 rounded-xl overflow-hidden bg-white max-h-32 flex items-center justify-between p-2">
                            <img src="{{ asset('storage/' . $pesticideShop->signature) }}" alt="Signature Preview" class="max-h-28 w-auto object-contain rounded-lg">
                            <span class="text-[10px] text-emerald-700 font-bold px-2 py-1 bg-emerald-50 rounded-lg">✓ Current</span>
                        </div>
                    @else
            ...
            input.addEventListener('change', function() {
                ...
                if (file) {
                    ...
                    reader.onload = (e) => {
                        ...
                        const contextBadge = container.querySelector('span');
