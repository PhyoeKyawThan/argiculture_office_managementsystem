                    ...
                    @foreach(['store_front' => 'အရှေ့ဘက် (Store Front)', 'store_end' => 'အနောက်ဘက် (Store End)', 'store_south' => 'တောင်ဘက် (Store South)', 'store_north' => 'မြောက်ဘက် (Store North)'] as $key => $label)
                        <div class="p-4 border border-slate-100 bg-slate-50/30 rounded-2xl space-y-3">
                            ...
                            <div class="grid sm:grid-cols-3 gap-3">
                                <div>
                                    <input type="text" name="surrounding_agreements[boundaries][{{ $key }}][name]" value="{{ old("surrounding_agreements.boundaries.{$key}.name") }}" placeholder="အမည်" required class="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none bg-white focus:ring-2 focus:ring-emerald-500">
                                </div>
                                <div>
                                    <input type="text" name="surrounding_agreements[boundaries][{{ $key }}][nrc]" value="{{ old("surrounding_agreements.boundaries.{$key}.nrc") }}" placeholder="မှတ်ပုံတင်အမှတ်" required class="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none bg-white focus:ring-2 focus:ring-emerald-500">
                                </div>
                                <div>
