    ...
    <div class="grid lg:grid-cols-2 gap-6">
        ...
        <section class="bg-white rounded-2xl border border-emerald-100 p-6 shadow-sm">
            ...
            <form method="POST" action="{{ route('admin.inquiries.update', $inquiry) }}" class="space-y-4">
                ...
                <div>
                    <label for="status" class="block text-sm font-bold text-slate-700 mb-1">{{ __('messages.inquiries.status_field') }}</label>
                    <select name="status" id="status" required
                        ...
                        @foreach(\App\Models\AgriculturalInquiry::STATUSES as $status)
                            <option value="{{ $status }}" @selected(old('status', $inquiry->status) === $status)>
                                {{ $status === 'pending' ? __('messages.inquiries.status_pending') : __('messages.inquiries.status_answered') }}
                            </option>
                        @endforeach
                    </select>
