...
@section('content')
    <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-8">
        <div>
            <p class="text-xs font-bold uppercase tracking-widest text-emerald-600 mb-1">{{ __('messages.inspections.township_line', ['township' => $inspection->township]) }}</p>
            <h1 class="text-3xl font-black text-emerald-900">{{ $inspection->owner_name }}</h1>
            <p class="text-slate-600 mt-1">{{ __('messages.inspections.inspected_prefix', ['date' => $inspection->inspection_date->format('M j, Y')]) }}
    ...
    <div class="grid lg:grid-cols-2 gap-6 mb-6">
        <section class="bg-white rounded-2xl border border-emerald-100 p-6 shadow-sm">
            <h2 class="text-sm font-black uppercase tracking-wider text-emerald-800 mb-4">{{ __('messages.inspections.shop_details') }}</h2>
            <dl class="grid gap-3 text-sm">
                <div><dt class="text-slate-500">{{ __('messages.inspections.address') }}</dt><dd class="font-semibold">{{ $inspection->shop_address }}</dd></div>
                <div><dt class="text-slate-500">{{ __('messages.inspections.township') }}</dt><dd class="font-semibold">{{ $inspection->township }}</dd></div>
                <div><dt class="text-slate-500">{{ __('messages.inspections.inspector') }}</dt><dd class="font-semibold">{{ $inspection->inspector?->name ?? __('messages.common.em_dash') }}</dd></div>
                @if($inspection->license_expiry_date)
                    <div><dt class="text-slate-500">{{ __('messages.inspections.license_expires') }}</dt><dd class="font-semibold">{{ $inspection->license_expiry_date->format('M j, Y') }}</dd></div>
                @endif
            </dl>
        </section>
        <section class="bg-white rounded-2xl border border-emerald-100 p-6 shadow-sm">
            <h2 class="text-sm font-black uppercase tracking-wider text-emerald-800 mb-4">{{ __('messages.inspections.compliance') }}</h2>
        ...
        <section class="bg-white rounded-2xl border border-emerald-100 p-6 shadow-sm">
            ...
            <dl class="grid gap-4 text-sm">
                @if($inspection->raw_findings_notes)
                    <div><dt class="text-slate-500 mb-1">{{ __('messages.inspections.findings') }}</dt><dd class="font-medium text-slate-800 whitespace-pre-line">{{ $inspection->raw_findings_notes }}</dd></div>
                @endif
                @if($inspection->action_taken)
                    <div><dt class="text-slate-500 mb-1">{{ __('messages.inspections.action_taken') }}</dt><dd class="font-semibold">{{ $inspection->action_taken }}</dd></div>
                @endif
                @if($inspection->remarks)
                    <div><dt class="text-slate-500 mb-1">{{ __('messages.inspections.remarks') }}</dt><dd class="text-slate-700 whitespace-pre-line">{{ $inspection->remarks }}</dd></div>
                @endif
            </dl>
        </section>
    @endif
